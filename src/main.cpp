    ...
    try {
        ...
        if (action == "--sobel-x") {
            ...
            dst_image = canny(src_image, threshold1, threshold2);
                    ...
                    action == "--median-const" || action == "--median-cycle") {
            check_argc(argc, 4, 5);
            int radius = 1;
            if (argc == 5) {
            ...
            } else if (action == "--median-cycle") {
                char* buf= new char[10];
                for (int i = 1; i <= radius; ++i)
                {
                    dst_image = median_const(src_image, i);
                    
                    sprintf(buf,"%s%d%s","m",i,".bmp");
                    //cout<<buf;
                    save_image(dst_image, buf);
                }
